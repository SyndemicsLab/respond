...
int main(int argc, char** argv){
    ...
    // I hate this route and lambda. I really want to clean it up when I have time
    ...
    ([&inputs, &costs, &utils](const crow::request& req){
        ...
        for (auto [part_name, part_value] : file_message.part_map){
            if ("ConfigFile" == part_name){
                // Get file name
                config_file = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                init_cohort = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                entering_cohort = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                oud_trans = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                intervention_trans = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                intervention_inits = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                od_rates = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                fod_rates = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                bgm_rates = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                smr_rates = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                health_costs = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                od_costs = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                pharma_costs = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                treat_costs = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                bg_utils = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                setting_utils = part_value.headers.find("Content-Disposition")->second.params["filename"];
                ...
                // Get file name
                oud_utils = part_value.headers.find("Content-Disposition")->second.params["filename"];
        ...
        std::string enteringSampleIntervention = req.url_params.get("intervention");
        std::string enteringSampleOUD = req.url_params.get("oud");
        inputs.loadEnteringSamples(entering_cohort, enteringSampleIntervention, enteringSampleOUD);
    ...
    CROW_ROUTE (app, "/upload/config").methods(crow::HTTPMethod::Post)
    ([&inputs, &costs, &utils](const crow::request& req){
        ...
        for (auto [part_name, part_value] : file_message.part_map){
            if ("InputFile" == part_name){
                // Get file name
                uploaded_file_name = part_value.headers.find("Content-Disposition")->second.params["filename"];
    ...
    CROW_ROUTE (app, "/upload/initCohort").methods(crow::HTTPMethod::Post)
    ...
    CROW_ROUTE (app, "/upload/enterCohort").methods(crow::HTTPMethod::Post)
    ([&inputs](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
        ...
        inputs.loadEnteringSamples(uploaded_file_name, enteringSampleIntervention, enteringSampleOUD);

        return crow::response(crow::status::OK);
    });

    CROW_ROUTE (app, "/upload/oudTrans").methods(crow::HTTPMethod::Post)
    ([&inputs](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
    ...
    CROW_ROUTE (app, "/upload/interventionTrans").methods(crow::HTTPMethod::Post)
    ([&inputs](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
    ...
    CROW_ROUTE (app, "/upload/interventionInits").methods(crow::HTTPMethod::Post)
    ([&inputs](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
    ...
    CROW_ROUTE (app, "/upload/overdoseRates").methods(crow::HTTPMethod::Post)
    ([&inputs](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
    ...
    CROW_ROUTE (app, "/upload/fatalOverdoseRates").methods(crow::HTTPMethod::Post)
    ([&inputs](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
    ...
    CROW_ROUTE (app, "/upload/mortalityRates").methods(crow::HTTPMethod::Post)
    ([&inputs](const crow::request& req){
        std::string uploaded_file_name;
        ...
        for (auto [part_name, part_value] : file_message.part_map){
            if ("InputFile" == part_name){
                ...
                // Get file name
                smr_file_name = part_value.headers.find("Content-Disposition")->second.params["filename"];
    ...
    CROW_ROUTE (app, "/upload/healthcareCost").methods(crow::HTTPMethod::Post)
    ...
    CROW_ROUTE (app, "/upload/overdoseCost").methods(crow::HTTPMethod::Post)
    ([&costs](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
    ...
    CROW_ROUTE (app, "/upload/pharmaCost").methods(crow::HTTPMethod::Post)
    ([&costs](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
    ...
    CROW_ROUTE (app, "/upload/treatmentCost").methods(crow::HTTPMethod::Post)
    ([&costs](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
    ...
    CROW_ROUTE (app, "/upload/backgroundUtil").methods(crow::HTTPMethod::Post)
    ...
    CROW_ROUTE (app, "/upload/oudUtil").methods(crow::HTTPMethod::Post)
    ([&utils](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
    ...
    CROW_ROUTE (app, "/upload/settingUtil").methods(crow::HTTPMethod::Post)
    ([&utils](const crow::request& req){
        std::string uploaded_file_name;
        crow::multipart::message file_message(req);
        for (auto [part_name, part_value] : file_message.part_map){
    ...
    CROW_ROUTE (app, "/calculateCost").methods(crow::HTTPMethod::Post)
    ...
    CROW_ROUTE (app, "/calculateUtil").methods(crow::HTTPMethod::Post)
    ([&hist, &costs, &utils](const crow::request& req){
        Calculator::CostCalculator costCalculator(costs, utils, hist);
    ...
    CROW_ROUTE (app, "/download/writeHistory").methods(crow::HTTPMethod::Get)
    ([&hist, &inputs, &writer](){
        writer.setInterventions(inputs.getInterventions());
        writer.setOUDStates(inputs.getOUDStates());
        writer.setDemographics(inputs.getConfiguration().getDemographicCombosVecOfVec());
    ...
    CROW_ROUTE (app, "/download/writeCost").methods(crow::HTTPMethod::Post)
    ([&hist, &inputs, &costs, &utils, &writer](const crow::request& req){
        Calculator::CostCalculator costCalculator(costs, utils, hist);
        Data::Cost cost = costCalculator.calculateCost();
    ...
    CROW_ROUTE (app, "/download/writeUtility").methods(crow::HTTPMethod::Post)
    ([&hist, &inputs, &costs, &utils, &writer](const crow::request& req){
        Calculator::CostCalculator costCalculator(costs, utils, hist);
        Data::Utility utility = costCalculator.calculateUtility();
