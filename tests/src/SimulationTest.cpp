...
TEST(SimulationCreation, MainConstructor) {
    std::shared_ptr<MockDataLoader> mockLoader =
        std::make_shared<MockDataLoader>();

    EXPECT_CALL(*mockLoader, getDuration()).WillRepeatedly(Return(10));
    Respond sim(mockLoader);
    int expected = 10;
    EXPECT_EQ(sim.getDuration(), expected);
}
...
//     Eigen::Tensor<bool, 0> eq1 = (eVec(0) ==
//     sim.GetEnteringSamples()(0)).all(); Eigen::Tensor<bool, 0> eq2 = (o1 ==
//     sim.GetOUDTransitions()).all(); Eigen::Tensor<bool, 0> eq3 =
...
//     Matrix3d i1(2, 4, 2);
//     i1.setValues(
//         {{{0.256506, 0.102857}, {0.743494, 0.897143}, {1.0, 1.0}, {0.0,
//         0.0}},
//          {{0.256506, 0.102857}, {0.743494, 0.897143}, {1.0, 1.0}, {0.0,
//          0.0}}});
