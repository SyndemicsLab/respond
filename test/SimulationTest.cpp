...
TEST(Loading, InitialGroup){
    ...
    sim.Run();
    Eigen::Tensor<float, 3> result = sim.getHistory().stateHistory.at(0);
...
TEST(Run, EmptyRun){
    ...
    sim.Run();
    Eigen::Tensor<float, 3> actual = sim.getHistory().stateHistory.at(0);
...
TEST(Run, SingleStepRun){
    ...
    Eigen::Tensor<float, 3> actual0 = history.stateHistory.at(0);
    Eigen::Tensor<float, 3> actual1 = history.stateHistory.at(1);
...
TEST(Run, MultiStepRun){
    ...
    o1.setValues( {{{.85, .9}, {.15, .1}, {.1, .2}, {.9, .8}},
                   {{.85, .9}, {.15, .1}, {.1, .2}, {.9, .8}}} );
    Eigen::Tensor<float, 3> o2(2, 4, 2);
    o2.setValues( {{{.85, .9}, {.15, .1}, {.1, .2}, {.9, .8}},
                   {{.85, .9}, {.15, .1}, {.1, .2}, {.9, .8}}} );
    Eigen::Tensor<float, 3> o3(2, 4, 2);
    ...
    t1.setValues( {{{.8, .2}, {.9, .1}}, {{.8, .2}, {.9, .1}},
                   {{.05, .95}, {.15, .85}}, {{.05, .95}, {.15, .85}}} );
    Eigen::Tensor<float, 3> t2(4, 2, 2);
    t2.setValues( {{{.8, .2}, {.9, .1}}, {{.8, .2}, {.9, .1}},
                   {{.05, .95}, {.15, .85}}, {{.05, .95}, {.15, .85}}} );
    Eigen::Tensor<float, 3> t3(4, 2, 2);
    ...
    Eigen::Tensor<float, 3> actual0 = history.stateHistory.at(0);
    Eigen::Tensor<float, 3> actual1 = history.stateHistory.at(1);
    Eigen::Tensor<float, 3> actual2 = history.stateHistory.at(2);
    Eigen::Tensor<float, 3> actual3 = history.stateHistory.at(3);
