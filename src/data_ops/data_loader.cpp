...
namespace respond::data_ops {
        ...
        for (int e = 0; e < entering_sample_change_times.size(); ++e) {
            ...
            for (int i = 0; i < number_intervention_states; ++i) {
                ...
                for (int b = 0; b < number_behavior_states; ++b) {
                    ...
                    entering_sample.slice(offsets, extent) = StrVecToMatrix3d(
    ...
    DataLoaderImpl::LoadInterventionTransitionRates(const std::string &file) {
        Data::IDataTablePtr intervention_transitions_table =
            LoadDataTable(file);
    ...
    TimedMatrix3d
    DataLoaderImpl::LoadFatalOverdoseRates(const std::string &file) {
    ...
    Matrix3d DataLoaderImpl::LoadMortalityRates(
        const std::string &smr_file,
        const std::string &background_mortality_file) {
    ...
    Matrix3d DataLoaderImpl::CreateTransitionMatrix3d(
        const Data::IDataTablePtr &table, Dimension dimension, int timestep) {
    ...
    Matrix3d DataLoaderImpl::DoubleToMatrix3d(double value, int first_dimension,
                                              ...
                                              int third_dimension) {
        std::vector<double> transition_vector(
