...
Data::Cost CostCalculator::calculateCost() {
    ...
    cost.pharmaCost = this->calculateCost(
        this->history.stateHistory, this->costLoader.getPharmaceuticalCost());
    cost.treatmentCost =
        this->calculateCost(this->history.stateHistory,
                            this->costLoader.getTreatmentUtilizationCost());
    cost.nonFatalOverdoseCost =
        this->calculateOverdoseCost(this->history.overdoseHistory,
                                    this->costLoader.getNonFatalOverdoseCost());
    cost.fatalOverdoseCost =
        this->calculateOverdoseCost(this->history.fatalOverdoseHistory,
...
Data::Utility CostCalculator::calculateUtility() {
    Data::Utility util;
    util.backgroundUtility = this->calculateCost(
        this->history.stateHistory, this->utilityLoader.getBackgroundUtility());
    util.oudUtility = this->calculateCost(this->history.stateHistory,
                                          this->utilityLoader.getOUDUtility());
    util.settingUtility = this->calculateCost(
        this->history.stateHistory, this->utilityLoader.getSettingUtility());
    return util;
}
